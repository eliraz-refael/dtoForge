...
const dtoTemplate = `// Generated by DtoForge{{if .Config.Version}} {{.Config.Version}}{{end}} (Zod) - DO NOT EDIT
{{range .Imports}}{{.}}
{{end}}
...
export type {{.DTO.Name}} = {
{{range .DTO.Properties}}  {{toCamelCase .Name}}{{if not (isEffectivelyRequired .)}}?{{end}}: {{toTSType .Type .Nullable}};
{{end}}};

export const {{.DTO.Name}}Schema: z.ZodType<{{.DTO.Name}}> = z.object({
...
const indexTemplate = `// Generated by DtoForge{{if .Config.Version}} {{.Config.Version}}{{end}} (Zod) - DO NOT EDIT
// {{.PackageName}} - OpenAPI Schema Validators

{{range .DTOs}}export * from './{{toKebabCase .Name}}';
{{end}}
{{if .GenerateHelpers}}
// Zod is imported locally so the helper below can reference it, then re-exported
// for consumers (a re-export-only binding is not usable locally).
import { z } from 'zod';
export { z };
...
const singleFileTemplate = `// Generated by DtoForge{{if .Config.Version}} {{.Config.Version}}{{end}} (Zod) - DO NOT EDIT
// {{.PackageName}} - OpenAPI Schema Validators

import { z } from 'zod';

{{range .DTOs}}{{setCurrentDTO .Name .IsSelfRecursive}}
{{if .Description}}/**
 * {{.Description}}
 */
{{end}}
{{if .IsSelfRecursive}}// Schema: {{.Name}} (recursive)
export type {{.Name}} = {
{{range .Properties}}  {{toCamelCase .Name}}{{if not (isEffectivelyRequired .)}}?{{end}}: {{toTSType .Type .Nullable}};
{{end}}};
...
{{else if eq .Type "enum"}}// Enum: {{.Name}}
