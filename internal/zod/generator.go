...
func (g *ZodGenerator) generateSingleFile(dtos []generator.DTO, config generator.Config, genConfig GenerationConfig) error {
	...
	// Build template step by step with proper functions
	templateContent := `// Generated by DtoForge (Zod) - DO NOT EDIT
...
{{range .Properties}}  {{toCamelCase .Name}}: {{toZodType .Type .Nullable (not .Required)}},
{{end}}});
...
`
	...
	tmpl, err := template.New("zod-single-file-rebuild").Funcs(g.templateFuncs()).Parse(templateContent)
	if err != nil {
	...
	data := struct {
		...
		DTOs:            dtos,
		Config:          config,
		Imports:         []string{}, // Not using for now
		PackageName:     g.getPackageName(config),
