...
{{range .DTO.Properties}}  {{safePropertyName .Name}}{{if not (isEffectivelyRequired .)}}?{{end}}: {{codecTSType . (isEffectivelyRequired .)}};
{{end}}}
...
{{if .GenerateHelpers}}// io-ts ` + "`" + `t` + "`" + ` is already imported at the top of this file; bring in fp-ts
// locally so the helpers below can reference it, then re-export both for consumers.
import { isLeft, isRight } from 'fp-ts/Either';
export { t, isLeft, isRight };
